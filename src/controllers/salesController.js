...
export const createSale = async (req, res) => {
	try {
		...
		const { productId, quantity, unitPrice, guarantyValue, guarantyUnit } =
			req.body;
		...
		const formatted = {
			...
			date: sale.timestamp,
			guarantyValue: sale?.guarantyValue,
			guarantyUnit: sale?.guarantyUnit,
		};
	...
	try {
		...
		const formatted = sales.map((s) => {
			...
			return {
				...
				guarantyValue: s?.guarantyValue,
				guarantyUnit: s?.guarantyUnit,
				stockAfterSale: variant?.stockQuantity || 0,
				date: s.timestamp,
			};
		});
	...
	try {
		...
		const salesWithProfit = sales.map((sale) => {
			...
			const productPurchases = purchases
				.filter(
					(p) =>
						p.productId === sale.productId && p.timestamp <= sale.timestamp,
				) // only past purchases
				.map((p) => ({ ...p }));
...
export const updateSale = async (req, res) => {
	try {
		...
		const { quantity, unitPrice, guarantyValue, guarantyUnit } = req.body;
		const adminId = req.admin?.adminId;
